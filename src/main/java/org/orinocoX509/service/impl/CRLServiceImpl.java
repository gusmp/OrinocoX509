...
public class CRLServiceImpl implements CRLService 
{
	...
	private X509v2CRLBuilder addAuthorityKeyIdentifier(X509v2CRLBuilder crlGenerator, CRLField authorityKeyIdentifierField) throws IOException
	{
		if (authorityKeyIdentifierField != null)
		{
			...
			if ((akiField.getAuthorityKeyIdentifier() == true) && (akiField.getAuthorityIssuerSerialNumberCertificate() == true))
			{
				SubjectPublicKeyInfo subjectPublicKeyInfo = new SubjectPublicKeyInfo(
						(ASN1Sequence)new ASN1InputStream( new ByteArrayInputStream(CAKeyService.getCAPublicKey().getEncoded())).readObject());
				...
				authorityKeyId = new AuthorityKeyIdentifier(subjectPublicKeyInfo, 
						new GeneralNames(genName), CAKeyService.getCACertificate().getSerialNumber());
			}
	...
	private X509v2CRLBuilder addNextUpdate(X509v2CRLBuilder crlGenerator, CRLField timeNextUpdateField)
	{
	...
	private X509v2CRLBuilder addCRLDistributionPoint(X509v2CRLBuilder crlGenerator, CRLProfile crlProfile) throws CertIOException
	{
		...
		{
			List<CRLFieldValue> crldpFieldValues = crlDistributionPoint.getValues();
			...
			IssuingDistributionPoint idp = new IssuingDistributionPoint(new DistributionPointName(new GeneralNames(gn)), 
					crldpFieldValue.isOnlyContainsUserCerts(),
					crldpFieldValue.isOnlyContainsCACerts());
	...
	private CertificateStatusValues getCRLStatus(BigInteger serialNumber, String crlUrl) throws EngineException
	{
		...
		catch(CertificateException exc)
		{
			...
			throw new EngineException(EngineErrorCodes.COULD_NOT_BE_CREATED_CRL_FACTORY, 
					"Error creating the CertificateFactory (CRL) for serial number " + serialNumber + "\nDetails:\n" + exc.toString());
		}
		catch(CRLException exc)
		{
			log.error("Error generation the CRL from the url " + crlUrl + " for serial number " + serialNumber + "\nDetails:\n" + exc.toString());
			throw new EngineException(EngineErrorCodes.GENERATION_CRL_ERROR, 
					"Error generation the CRL from the url " + crlUrl + " for serial number " + serialNumber + "\nDetails:\n" + exc.toString());
		}
	}
