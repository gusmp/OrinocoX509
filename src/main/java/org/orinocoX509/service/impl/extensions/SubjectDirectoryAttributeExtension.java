...
public class SubjectDirectoryAttributeExtension implements EngineExtension
{
    ...
    public SubjectDirectoryAttributeExtension(List<BaseCertificateFieldValue> values, Boolean critical)
    {
	this.values = values;
	...
	for (int i = 0; i < this.values.size(); i++)
	{
	    ...
	    {
		Attribute atribute = new Attribute(new ASN1ObjectIdentifier(SubjectDirectoryAttributeType.COUNTRY_OF_RESIDENCE.toString()), new DERSet(new DERPrintableString(sdaFieldValue.getValue())));
	    ...
	    {
		Attribute atribute = new Attribute(new ASN1ObjectIdentifier(SubjectDirectoryAttributeType.COUNTRY_OF_CITIZENSHIP.toString()), new DERSet(new DERPrintableString(sdaFieldValue.getValue())));
		attributeVector.add(atribute);
	    }
	}
