...
public class TestSupport 
{
	...
	public CertificateProfile generateBaseProfile(String profileName, String profileDescription)
	{
		...
		CertificateField subject = new SubjectField(profile, TestConst.SUBJECT_PATTERN);
		CertificateField issuer = new IssuerField(profile, TestConst.ISSUER_PATTERN);
	...
	public CertificateValues generateBaseValues()
	{
	...
	public String getValue(CertificateFieldValue fieldValue, boolean isKey, boolean isSubject)
	{
		if (isSubject == true)
		{
			if (isKey == true)
	...
	public void checkFieldValue(List<CertificateFieldValue> certificateFieldValues, List<String> expectedValues, 
			boolean isKey, boolean isSubject)
	{
		boolean contain;
	...
	public X509Certificate getCertificate(String pemCertificate) 
	{
		try
		{
			...
			return((X509Certificate) certificateFactory.generateCertificate(new ByteArrayInputStream(cryptographicUtil.base64ByteDecode(pemCertificate))));
		}
		catch(Exception exc)
		{
			return(null);
	...
	/*
	public CRLProfile generateBaseCRLProfile(String profileName, String profileDescription)
	{
		CRLProfile profile = new CRLProfile(profileName, profileDescription, TestConst.MAX_DAYS_REVOKED_CERTIFICATES);
		return(profile);
	}
	*/
